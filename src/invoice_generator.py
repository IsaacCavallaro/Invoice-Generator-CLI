...
def draw_invoice_table(
    can, item_details, amount, start_x, start_y, available_width, table_height
):
    table_width = available_width
    amount_with_prefix = f"${amount}"

    item_total_data = [["ITEM", "AMOUNT"], [item_details, amount_with_prefix]]
    table = Table(
        item_total_data,
        colWidths=[table_width / 2] * 2,
        rowHeights=[table_height / 2] * 2,
    )
    table.setStyle(
        TableStyle(
            [
...
def generate_invoice_pdf(invoice_data, template_file, output_file):
    ...
    draw_billed_to(
        can, invoice_data["billed_to"], start_x, billed_to_start_y, available_width
    )

    draw_invoice_table(
        can,
        invoice_data["item_details"],
        invoice_data["amount"],
        start_x,
        table_start_y,
        available_width,
...
def send_invoice_email(
    sender_email, sender_password, receiver_email, pdf_file, invoice_number
):
...
def save_email_address(name, email):
    ...
    with open("src/email_addresses.json", "w") as file:
        json.dump(email_addresses, file, indent=4)


def load_email_addresses():
    try:
        with open("src/email_addresses.json", "r") as file:
...
def choose_or_enter_email():
    ...
    receiver_email_choice = input(
        "Choose an email address (enter the key) or type a new email: "
    )
